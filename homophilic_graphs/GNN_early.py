...
class GNNEarly(BaseGNN):
  def __init__(self, opt, dataset, device=torch.device('cpu')):
    ...
    self.weight_v = nn.Parameter(torch.Tensor(opt['hidden_dim'], opt['hidden_dim']), )
    self.v_pa = nn.Parameter(torch.Tensor(dataset.data.x.shape[0], opt['hidden_dim']),)
    # self.v_pa = self.v_pa.to(device)
    self.reset_parameters_v()
  ...
  def reset_parameters_v(self):
    ...
    init.constant_(self.weight_v, 0.1)
  def forward(self, x):
    ...
    # drop_weight = self.weight_v.to(self.device)
    # vt = x @ drop_weight.transpose(-1, -2)
    if self.opt['dataset'] == 'airport':
      # print("learned V parameter")
      vt = self.v_pa
    else:
      drop_weight = self.weight_v.to(self.device)
      vt = x @ drop_weight.transpose(-1, -2)
      # vt =x.clone()
    # vt = torch.zeros(x.shape).to(self.device)
    x = torch.cat([x, vt], dim=-1)
    # x = torch.cat([x, x], dim=-1)
    # x = torch.cat([x, self.v_pa], dim=-1)
    ...
    #     end_time = time.time()
    #
    #     print("aveage time: ", (end_time -start_time)/10 )
