...
def main(cmd_opt):
    ...
    for rep in range(opt['num_splits']):
        ...
        np.save(os.path.join(save_dir, 'labels' + str(opt['dataset']) + '.npy'), labels)
        for epoch in range(1, opt['epoch']):
            start_time = time.time()

            accs, logits = this_test(model, data, opt)
            tmp_train_acc, tmp_val_acc, tmp_test_acc = accs[0], accs[1],accs[2]
            ...
            if tmp_val_acc > val_acc:
                ...
                np.save(os.path.join(save_dir, 'embeddings' + str(opt['dataset']) + '.npy'), logits)
        ...
        torch.save(model.state_dict(),
                   os.path.join(save_dir, 'model_' + str(opt['dataset']) + str(opt['time']) + '.pth'))
        if opt['num_splits'] > 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--runtime', type=int, default=1, help='running time')
    parser.add_argument('--seed', type=int, default=12345, help='running time')
    ...
    for _ in range(args.runtime):
        _, _ ,test_acc,opt_final = main(opt)
        opt['seed'] = opt['seed'] + 10
